...
export class Signal<T, K extends string | undefined = undefined> extends BehaviorSubject<T> {
  constructor(
    public config: {
      default: T;
      key: K;
...
export function signal<T, K extends string | undefined = undefined>(
  ...
  return new Signal({ key: undefined, ...config, default: config.default ?? undefined });
}

export function virtualSignal<T>(config: { default: T; key?: string }): VirtualSignal<T> {
