...
it("should compare keys", () => {
  ...
  expect(compareKeys([], ["foo"], { exact: false })).toBe(false)
  expect(compareKeys(["foo"], [], { exact: false })).toBe(true)
  expect(
    compareKeys(["foo", "bar"], ["foo"], { exact: false })
  ).toBe(true)
  expect(compareKeys(["foo"], ["foo", "bar"], { exact: true })).toBe(false)
  expect(compareKeys(["foo", "bar"], ["foo"], { exact: false })).toBe(true)
  expect(compareKeys(["foo"], ["foo", "bar"], { exact: false })).toBe(false)
  expect(
