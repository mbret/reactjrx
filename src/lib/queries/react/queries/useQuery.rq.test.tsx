...
import { mockOnlineManagerIsOnline, mockVisibilityState, queryKey } from "../../client/tests/utils"
...
describe("useQuery", () => {
  ...
  it("should throw error if queryFn throws and throwOnError is in use", async () => {
    ...
    const rendered = renderWithClient(
      queryClient,
      <ErrorBoundary fallbackRender={(c) => <div>error boundary</div>}>
        <Page />
      </ErrorBoundary>
    )
  ...
  describe("networkMode always", () => {
    it("always queries should start fetching even if you are offline", async () => {
      ...
      await waitFor(() => {
        expect(
          rendered.getByText("data: data 1")
          // @ts-ignore
        ).toBeInTheDocument()
      })
    ...
    it("always queries should not pause retries", async () => {
      ...
      await waitFor(() => {
        expect(
          rendered.getByText("error: error 2")
          // @ts-ignore
        ).toBeInTheDocument()
      })
